...
export async function GET(request: Request) {
  ...
  const stateCookie = parseStateCookie((await cookies()).get("meli_oauth_state")?.value);
  ...
  try {
    const connection = await connectMercadoLivreAccount(stateCookie.orgId, code);
    ...
    if (session) {
      await writeAudit({
        ...
        after: { provider: "MERCADO_LIVRE", externalUserId: connection.externalUserId },
      });
    }
